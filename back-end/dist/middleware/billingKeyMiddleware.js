...
async function payNowAndRecord(req, res, next) {
    try {
        ...
        const newPlan = res.locals.newPlan;
        const newPlanPrice = res.locals.newPlanPrice;
        const newPlanGrant = res.locals.newPlanGrant;
        if (!flags)
        ...
        if (!subscriptionId)
            return next(new Error("subscriptionId 필요"));
        const billingKey = res.locals.user?.portone_billing_key;
        const amount = flags === "UPGRADE"
            ? newPlanPrice ?? (newPlan ? all_Types_1.PLAN_ITEMS[newPlan].price : null)
        ...
        try {
           ...
           ON DUPLICATE KEY UPDATE
            ...
            if (flags === "UPGRADE") {
                ...
                const price = newPlanPrice ?? all_Types_1.PLAN_ITEMS[targetPlan].price;
                const grant = newPlanGrant ?? all_Types_1.PLAN_ITEMS[targetPlan].token_grant;
              ...
              SET plan_name         = ?,
                  ...
                  updated_at            = NOW()
            WHERE id = ?`, [targetPlan, price, grant, subscriptionId]);
                if (grant > 0) {
                    await conn.query(`UPDATE users SET token_balance = token_balance + ? WHERE id = ?`, [grant, userId]);
                }
                // 최신 상태를 다음 미들웨어에서 쓰도록 로드
                const [[updatedSub]] = await conn.query(`SELECT * FROM subscriptions WHERE id = ? LIMIT 1`, [subscriptionId]);
                const [[updatedUser]] = await conn.query(`SELECT * FROM users WHERE id = ? LIMIT 1`, [userId]);
