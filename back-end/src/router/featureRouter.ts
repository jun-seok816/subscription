...
export function featureRouter() {
  ...
  const labels = Array.from(
    new Set(Object.values(PLAN_ITEMS).flatMap((p) => p.features.map((f) => f.label)))
  );

  for (const label of labels) {
    const path = `/feature/${toSlug(label)}`;

    router.post(path, process._myApp.checkSession ,async (req, res) => {
      const userId = Number((req.session.userId)); 
      try {
        const [[sub]]: any = await process._myApp.db.promise().query(
          "SELECT plan_name FROM subscriptions WHERE user_id = ?",
          [userId]
        );
        const planName = (sub?.plan_name?.toUpperCase?.() ?? "FREE") as keyof typeof PLAN_ITEMS;
        const plan = PLAN_ITEMS[planName] ?? PLAN_ITEMS.FREE;
        ...
        const [r]: any = await process._myApp.db.promise().query(
