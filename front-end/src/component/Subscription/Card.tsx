...
function getBrandMeta(brand: string | null): {
  ...
  if (!brand) {
    return { label: "등록된 결제수단 없음", kftc: null, iconPath: null };
  }

  const kftc = KFTC_CODE_BY_BRAND[brand] ?? null;
  const label = BRAND_LABEL[brand] ?? brand;
  const iconPath = kftc ? `/assets/images/icons/${kftc}.png` : null;
      ...
      <td>
        <div className="ubt__paymentRow">
          ...
          <button
            className="btn__red"
            onClick={async () => {
              if (window.confirm("등록된 결제수단을 삭제하시겠습니까?")) {
